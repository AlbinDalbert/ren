...
const projectRecords = [
  {
    ...
    status: 'active',
    summary: 'Rust CLI/library for file-backed HTML projects: validation, metadata, notes, links, search, and graph indexes.',
  ...
  },
  {
    ...
    path: '~/apps/amanite',
    status: 'active',
    summary: 'Desktop editor for opening Fractal projects, writing pages, saving through the engine, and inspecting local graph state.',
...
function VariantHeader({ active, label }: { active: ArchiveVariantKey; label: string }) {
  return (
    <header className="variant-header" aria-label={`${label} navigation`}>
      <a className="variant-brand" href="?demo=archive" aria-label="Return to archive demo 02">
        <span className="variant-brand-mark" aria-hidden="true">REN</span>
        <span>
      ...
      <nav className="variant-switcher" aria-label="Archive homepage variants">
        ...
        {variantLinks.map(([key, text]) => (
          <a href={`?demo=archive-${key}`} aria-current={active === key ? 'page' : undefined} key={key}>
            {text}
          </a>
        ))}
      </nav>
...
function LedgerTable() {
  return (
    <div className="variant-ledger-table" role="table" aria-label="Archive commitments">
    ...
    <main className="variant-home variant-atlas" id="variant-top">
      ...
      <section className="atlas-hero" aria-labelledby="atlas-title">
        <div className="atlas-copy">
          ...
          <p>
            A homepage as cartographic index: projects are places, links are routes, and the durable artifact is always visible as a coordinate.
          </p>
          <div className="variant-actions">
            <a className="variant-button variant-button-primary" href="#atlas-map">Open atlas</a>
            <a className="variant-button" href="#atlas-records">Inspect records</a>
          </div>
        </div>

        <div className="atlas-map" id="atlas-map" aria-label="Fractal and Amanite map">
          <span className="atlas-coordinate atlas-coordinate-nw">N 59.33 / E 18.06</span>
          <span className="atlas-coordinate atlas-coordinate-se">REN GRAPH / LOCAL</span>
          <div className="atlas-route atlas-route-main" aria-hidden="true" />
          <div className="atlas-route atlas-route-secondary" aria-hidden="true" />
      ...
      <section className="variant-section" id="atlas-records" aria-labelledby="atlas-records-title">
        <div className="variant-section-heading">
          <p className="variant-kicker">Mapped records</p>
          <h2 id="atlas-records-title">Two coordinates define the current territory.</h2>
        </div>
        <VariantProjectCards />
      </section>
    </main>
  ...
  return (
    <main className="variant-home variant-ledger" id="variant-top">
      ...
      <section className="night-ledger-hero" aria-labelledby="ledger-title">
        <aside className="ledger-spine" aria-label="Ledger volume">
          <span>volume</span>
          <strong>04</strong>
          <em>public index</em>
        </aside>

        <div className="ledger-stack" aria-label="Stacked ledger documents">
          <article className="ledger-page">
            <div className="ledger-page-meta">
              <span>path: /ren/home</span>
              <span>status: unsealed draft</span>
            </div>
            <p className="variant-kicker">Night Ledger</p>
            <h1 id="ledger-title">A homepage with the weight of a kept record.</h1>
            <p>
              The public face becomes quieter and more literary: an index page held inside dark operator chrome, built for notes, releases, and durable project entries.
            </p>
            <LedgerTable />
          </article>
        </div>
      ...
      <section className="variant-section" id="ledger-records" aria-labelledby="ledger-records-title">
        <div className="variant-section-heading">
          <p className="variant-kicker">Filed entries</p>
          <h2 id="ledger-records-title">Project entries read like durable catalog cards.</h2>
        </div>
        <VariantProjectCards />
      </section>
    </main>
    ...
    <main className="variant-home variant-stacks" id="variant-top">
      ...
      <section className="stacks-hero" aria-labelledby="stacks-title">
        <div className="stacks-copy">
          ...
          <p>
            A more physical metaphor: the homepage becomes an archive wall where Fractal and Amanite are folders inside a growing repository.
          </p>
          <div className="variant-actions">
            <a className="variant-button variant-button-primary" href="#stacks-catalog">Open catalog</a>
            <a className="variant-button" href="?demo=archive-ledger">Compare ledger</a>
          </div>
        </div>

        <div className="stacks-wall" aria-label="Repository shelves">
          {projectRecords.map((project, index) => (
            <a className="stack-folder" href="#stacks-catalog" style={{ '--folder-offset': `${index * 20}px` } as CSSProperties} key={project.id}>
              <span>{project.id}</span>
      ...
      <section className="variant-section" id="stacks-catalog" aria-labelledby="stacks-catalog-title">
        <div className="variant-section-heading">
          <p className="variant-kicker">Catalog</p>
          <h2 id="stacks-catalog-title">Each project is a drawer with visible labels.</h2>
        </div>
        <VariantProjectCards />
      </section>
    </main>
